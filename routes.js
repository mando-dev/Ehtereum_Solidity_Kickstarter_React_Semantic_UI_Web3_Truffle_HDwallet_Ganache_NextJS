...
routes  
    ...
    .add('/campaigns/:address','/campaigns/show')   //routes we want to have inside our application. //settgin up the routing rule. the add() func is how we define a new route mapping. first arg is the pattern we want to look for- a wildcard. wilcard = ':address'. address is just a name we created here first. 2nd arg is to show whcih Component do want to show from our 'pages directory whenver someone is directed to the first arg. 'address' aka token.    
    .add('/campaigns/:address/requests','/campaigns/requests/index')  //setting up dynamic routes to look for urls with an 'address0xuj7233/requests'. First arg is 'if anyone lands here', 2nd arg is where we want to route them once they land on arg 1. 
    .add('/campaigns/:address/requests/new', '/campaigns/requests/new'); //2nd arg takes them to the comopoent 'RequestNew?
